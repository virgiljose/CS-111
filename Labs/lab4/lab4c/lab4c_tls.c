...
/* header files for tls implementation */
...
/* additional variables for tls implementation */
BIO * certbio = NULL;
BIO * outbio = NULL;
X509 * cert = NULL;
...
void init_connection() {
    ...
    if(SSL_library_init() < 0){
        fprintf(stderr, "Failed to initialize OpenSSL library\n");
        exit(1);
    }
    ...
    if (ctx == NULL) {
        fprintf(stderr, "Failed to create new SSL context structure\n");
        exit(1);
    }
    
    ssl = SSL_new(ctx);
    if (ssl == NULL) {
        fprintf(stderr, "Failed to create new SSL connection state object\n");
        exit(1);
    }
    ...
    if (SSL_connect(ssl) < 1) {
        fprintf(stderr, "Failed to build SSL session\n");
        exit(1);
    }
    ...
    if(logfd != NULL)
        fprintf(logfd, "ID=%s\n", userID);
    char ID_message[13];
    strcpy(ID_message, "ID=");
    strcat(ID_message, userID);
    strcat(ID_message, "\n");
    fprintf(stderr, "%s", ID_message);
    SSL_write(ssl, ID_message, 13);
